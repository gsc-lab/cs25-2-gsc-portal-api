...
// 강의실 개방 투표 생성
export async function postClassroomPolls(grade_id, classroom_id, poll_date, target_weekend, required_count) {
...
export async function addVote(user_id, poll_id) {
    ...
    if (exists.length > 0) {
        const err = new Error("이미 투표에 참여했습니다.");
        err.statusCode = 400;
        throw err;
    }
...
export async function removeVote(user_id, poll_id) {
    ...
    if (result.affectedRows === 0) {
        const err = new Error("해당 투표에 참여하지 않았습니다.");
        err.statusCode = 404;
        throw err;
    }
