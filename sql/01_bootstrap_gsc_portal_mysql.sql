-- =========================================================
-- GSC Portal Database Schema (Cascading & Tx-safe)
-- =========================================================
...
-- 안전을 위해 존재 시 드롭하고 새로 생성하려면 주석 해제
-- SET FOREIGN_KEY_CHECKS=0;
-- DROP TABLE IF EXISTS
--   notice_file, notice_target, notification_delivery_notice, notification_delivery_event,
--   course_event, course_language, course_target, course_schedule, course_student, course_professor,
--   student_exams, reservation, weekend_attendance_votes, weekend_attendance_poll,
--   cleaning_assignment, log_entity, allowed_email, kakao_user, user_role, student_entity,
--   file_assets, notice, course, classroom, time_slot, section,
--   level_class, level, grade, language, user_account;
...
CONSTRAINT fk_nt_level  FOREIGN KEY (level_id)    REFERENCES level(level_id)      ON UPDATE CASCADE ON DELETE SET NULL,
...
CONSTRAINT fk_exam_user  FOREIGN KEY (user_id)  REFERENCES user_account(user_id) ON UPDATE CASCADE ON DELETE CASCADE,
CONSTRAINT fk_exam_file  FOREIGN KEY (file_id)  REFERENCES file_assets(file_id)  ON UPDATE CASCADE ON DELETE SET NULL,
CONSTRAINT fk_exam_level FOREIGN KEY (level_id) REFERENCES level(level_id)       ON UPDATE CASCADE ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
